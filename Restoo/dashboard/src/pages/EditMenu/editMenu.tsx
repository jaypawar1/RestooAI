...
const EditMenu = () => {
    ...
    const ImagetoBase64 = (file: File | undefined): Promise<string | ArrayBuffer | null> => {
    ...
    const handleInputChange = (
        ...
        if (typeof updatedMenu.category[categoryIndex].item[itemIndex][field] === 'string') {
            updatedMenu.category[categoryIndex].item[itemIndex][field] = updatedMenu.category[categoryIndex].item[itemIndex][field] = value;;
        } else {
    ...
    const handleFileChange = async (categoryIndex: number, itemIndex: number, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleAddMoreItems = (category: string) => {
        const updatedMenu = { ...menu };
        const categoryIndex = updatedMenu.category.findIndex(cat => cat.name === category);
        if (categoryIndex !== -1) {
    ...
    const handleCreateMenu = async (menu: Menu) => {
        try {
            if (check) {
                const response = await axios.put("http://localhost:5000/api/menu", menu, {
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
                        Authorization: token!,
    ...
    const handleOnsubmit = async (item: MenuItem[]) => {
        try {
            ...
            if (user.data.menu) {
                const response = await axios.put("http://localhost:5000/api/menu", item, {
                    headers: {
                        Accept: 'application/json',
                        'Content-Type': 'application/json',
                        Authorization: token!,
        ...
        <>
            <DefaultLayout>
                ...
                <div className='w-full flex justify-between'>
                    ...
                    <button className='text-white bg-[#4BC500] font-semibold px-3 py-2 text-xl rounded-xl'>
                        POS integration
                    </button>
                </div>
                <div className='text-xl mt-10 w-[60%]'>
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum nemo, ratione omnis, quibusdam ea minima error, provident aperiam quam fuga optio distinctio rerum corporis voluptatibus voluptatum ab totam enim mollitia?
                </div>
                <div className='w-[70%] mt-20'>
                    <div className='flex gap-4'>
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Restaurant Name</label>
                            <input
                                type='text'
                ...
                <div className='mt-5 w-[60%] '>
                    <label className='text-black font-medium text-2xl'>Create your item categories</label>
                    <div className=' mt-4 grid grid-cols-3 gap-2'>
                        {menu.category.map((catagory, index) => (
                            <div key={index} onClick={() => toggleCategorySelection(catagory.name)} className={`bg-[#BAE8B3] w-full text-black text-lg font-medium rounded-xl py-4 text-center col-span-1 shadow-2 drop-shadow-xl  ${selectedCategories.has(catagory.name) ? ' bg-[#BAE8B3]' : 'bg-white'}`}>
                                {catagory.name}
                            </div>
                        ))}
                        <div onClick={() => {
                            const newCategory = window.prompt("Enter the name of the new service:");
                            if (newCategory) {
                                setMenu(prevMenu => ({
                                    ...prevMenu,
                                    category: [
                                        ...prevMenu.category,
                                        { name: newCategory, item: [] } // Ensure 'item' property is initialized as an empty array
                                    ]
                                }));
                            }
                        }} className=' w-full text-black text-lg font-medium rounded-xl py-4 text-center col-span-1 shadow-2 drop-shadow-xl flex justify-between px-4 '>
                ...
                <div className='mt-5 w-[60%] '>
                    <label className='text-black font-medium text-2xl'>Add your Items</label>
                    <div>Lorem ipsum dolor sit amet consectetur adipisicing elit. Vitae cum vero earum reiciendis, praesentium numquam adipisci amet porro modi! Nobis.</div>
                    <div className=' mt-4 '>
                        {[...selectedCategories].map((category, catIndex) => (
                            <div key={catIndex} className='mb-10'>
                                <div>
                                    <label className='text-black font-medium text-xl mb-5'>{category}</label>
                                    {menu.category
                                        .filter(cat => cat.name === category)
                                        .flatMap(cat => cat.item)
                                        .map((item, itemIndex) => (
                                            <div key={itemIndex} className='bg-[#BAE8B3] mt-5 w-full text-black text-lg font-medium rounded-xl py-4 px-4 text-center shadow-2 drop-shadow-xl mb-10'>
                                                <div className='flex flex-col mt-5'>
                                                    <div className='flex justify-between gap-4'>
                                                        ...
                                                        <input
                                                            ...
                                                            type='file'
                                                            className='hidden'
                                                            onChange={(e) => handleFileChange(catIndex, itemIndex, e)}
                                                        />
                                                        <input
                                                            type='number'
                                                            placeholder="Amount"
                                ...
                                <div className='mt-6'>
                                    <div onClick={() => handleAddMoreItems(category)} className='text-[#4BC500] underline mt-6 cursor-pointer'>
                                        Add more items
                                    </div>
                                </div>
                                <div className='mt-6'>
                        ...
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Restaurant address google maps link</label>
                            <input
                                type='text'
                                value={menu.map}
                                onChange={(e) => {
                        ...
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Contact number</label>
                            <input
                                type='phone'
                                value={menu.phone}
                                onChange={(e) => {
                        ...
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Email address</label>
                            <input
                                type='email'
                                value={menu.email}
                                onChange={(e) => {
                        ...
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Contact address</label>
                            <input
                                type='address'
                                value={menu.address}
                                onChange={(e) => {
                                ...
                                placeholder='Titoes cafe and Bar'
                                className='rounded-xl border text-xl font-medium border-black h-12 py-2 px-3 mt-3 h-20'
                            />
                        </div>
                        <div className='flex flex-col w-[70%] '>
                            <label className='text-black font-medium text-2xl'>Instagram</label>
                            <input
                                type='name'
                                value={menu.insta}
                                onChange={(e) => {
                ...
                <button onClick={() => handleCreateMenu(menu)} className='text-white bg-[#4BC500] font-semibold px-3 py-2 text-xl rounded-xl mt-6'>
                    Create Menu
                </button>
            </DefaultLayout >
        </>
