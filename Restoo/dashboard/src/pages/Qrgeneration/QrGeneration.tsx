    ...
    <>
      <DefaultLayout>
        ...
        <div className="grid grid-cols-2 gap-10 ">
                ...
                <div className="grid grid-cols-3 gap-y-2 gap-x-3 w-fit">
                  {tables && tables.length > 0 ? (
                    tables.map((table, index) => (
                      <div
                        key={index}
                        className={`border-2 rounded-lg w-full h-10 p-3 flex justify-center items-center cursor-pointer ${selectTable === table._id
                            ? 'border-blue-500'
                            : 'border-[#4bc500]'
                          }`}
                        onClick={(e) => {
          ...
          <div>
            <div className="bg-slate-500 h-39"></div>
              ...
              {tables && tables.length > 0 ? (
                tables.map((table, index) => (
                  <div className="flex flex-row justify-between items-start m-5">
                    <div
                      className="inline-block pr-6 text-center border border-gray-300"
                    >
                      <div className="text-2xl mb-2">{restaurantName}</div>
                      <QRCodeCanvas value={`https://wa.me/917709196986?text=Hello,%20I'd%20like%20to%20contact%20${restaurantName}%20regarding%20table%20${table.section}`} size={156} className='ml-5' />
            ...
            <div className="mt-4 flex justify-center">
              <button
                ...
                className="bg-slate-500 text-white px-4 py-2 rounded-md hover:bg-gray-600"
              >
                Show More
              </button>
            </div>
